...
namespace quantas {
    ...
    void PBFTPeer::performComputation() {
        ...
        // maxCrashes is achieved either earlier or once 90% of rounds are done
        const int randNum = randMod(100);
        int chance = (getRound() / (getLastRound() * 0.90)) * 100;
        if ((id() < maxCrashes) && !crashed && (randNum < chance)) {
            crashed = true;
        }
        if (crashed) { return; }
    ...
    void PBFTPeer::checkContents() {
        ...
        } else if ((receivedMessages.size() - 1) == sequenceNum) {
            PBFTPeerMessage message = receivedMessages[sequenceNum].back();
            if (message.type == messageType::new_view) { // go to new view
                timer = 0;
                viewJumps = 0;
